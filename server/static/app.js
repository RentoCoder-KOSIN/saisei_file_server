...
function saveSession() {
    sessionStorage.setItem('pylab_user', currentUser);
    sessionStorage.setItem('pylab_role', currentRole);
}

function clearSession() {
    sessionStorage.removeItem('pylab_user');
    sessionStorage.removeItem('pylab_role');
}
...
window.addEventListener('DOMContentLoaded', () => {
    const savedUser = sessionStorage.getItem('pylab_user');
    const savedRole = sessionStorage.getItem('pylab_role');
    if (savedUser && savedRole) {
        applyLoginUI(savedUser, savedRole);
    }
});
...
async function login() {
    ...
    saveSession();
    applyLoginUI(data.username, data.role);
...
function onDrop(e) {
    ...
    // webkitGetAsEntry() でフォルダのパス情報を正しく取得する
    const items = e.dataTransfer.items;
    if (items && items.length > 0) {
        collectFilesFromItems(items).then(files => {
...
// dataTransfer.items からファイルを再帰収集（フォルダ対応）
function collectFilesFromItems(items) {
    ...
    return Promise.all(entries.map(readEntry)).then(results => results.flat());
}
...
function readEntry(entry) {
    if (entry.isFile) {
        return new Promise((resolve) => {
            entry.file(file => {
                // webkitRelativePath は読み取り専用なので代替プロパティで保持
                Object.defineProperty(file, 'webkitRelativePath', {
                    get: () => entry.fullPath.replace(/^\//, ''),
                    configurable: true,
                });
                resolve([file]);
            });
...
async function runFile(name) {
    ...
    if (!res.ok) {
        ...
        showResult(name, '', res.statusText || '実行エラー');
        return;
    }
...
function showResult(name, stdout, stderr) {
    ...
    const stderrLines = [];
    (stderr || '').split('\n').forEach(line => {
        if (line.startsWith('[debug]') || line.startsWith('[exit error]') || line.startsWith('[docker]') || line.startsWith('[no-docker]')) {
            debugLines.push(line);
        } else if (line.trim()) {
            stderrLines.push(line);
        }
    });
    const cleanStderr = stderrLines.join('\n');
    ...
    panel.innerHTML = `
        ...
        <pre class="result-stdout">${stdout ? escHtml2(stdout) : '// 出力なし'}</pre>
        ${cleanStderr ? `<pre class="result-stderr">${escHtml2(cleanStderr)}</pre>` : ''}
        ${debugInfo  ? `<pre class="result-debug" style="font-size:10px;opacity:0.5;white-space:pre-wrap;">${escHtml2(debugInfo)}</pre>` : ''}
    `;
...
function renderTasks(tasks) {
    ...
    const isTeacher = document.body.classList.contains('is-teacher');
    const visible = tasks; // 全課題を表示（複数提出対応）
    ...
    list.innerHTML = visible.map((t, i) => {
        ...
        const submittedBadge = (!isTeacher && mySubmittedTaskIds.has(t.id))
            ? `<span class="task-badge" style="background:rgba(100,200,100,0.15);color:#4caf50;margin-left:4px;">✅ 提出済（再提出可）</span>`
            : '';
        return `
        <div class="task-item${t.is_expired ? ' expired' : ''}" style="animation-delay:${i * 0.05}s">
            <div class="task-main">
                <div class="task-title">${t.title} ${badge}${submittedBadge}</div>
        ...
        <div class="modal-card">
            ...
            <div class="modal-body">
                ...
                <div class="status-row ${s.status}">
                    ...
                    <span class="status-badge ${s.status}">${statusLabel(s.status)}${s.count > 1 ? ` (${s.count}件)` : ''}</span>
