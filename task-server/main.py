...
def submission_status(task_id: int):
    ...
    # 複数提出対応: ユーザーごとに全提出を収集
    submitted_map = {}
    submitted_all = {}
    for s in submissions:
        uname = s["username"]
        if uname not in submitted_map:
            submitted_map[uname] = dict(s)
        if uname not in submitted_all:
            submitted_all[uname] = []
        submitted_all[uname].append(dict(s))

    result = []
    for uname, s in submitted_map.items():
        result.append({
            "username":        uname,
            "status":          "late" if s["is_late"] else "submitted",
            "submitted_at":    s["submitted_at"],
            "filename":        s["filename"],
            "all_submissions": submitted_all[uname],
            "count":           len(submitted_all[uname]),
        })
